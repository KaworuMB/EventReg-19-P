...
def dispatcher(request):
    ...
    if sender == "delete":
        ...
        messages.warning(request, "Your registrations for the event \"" + event.event_name + "\" have been deleted")
    ...
    if sender == "edit_info":
        ...
        try:
            ...
            messages.error(request, "Save error — the input might be too long. Please try shortening it!")
    ...
    if sender == "edit_creds":
        ...
        if user is not None:
            try:
                ...
                messages.error(request, "The password is too weak. Please follow the password requirements")
                return redirect(profile)
            if request.POST["new"] != request.POST["repeat"]:
                messages.error(request, "Repeated password does not match the new one")
                return redirect(profile)
    ...
    if sender == "signup":
        ...
        try:
            send_mail(
                subject='Account in the SDU Registration System',
                message='You have successfully created an account in the SDU event registration system . Choose your events — we’ll see you there!\n\nDon’t forget to fill out your profile if you want to receive personalized certificates.\n\nYour current password: ' + password,
        ...
        messages.success(request, "Login instructions have been sent to your email")
    ...
    if sender == "forgot":
        ...
        if filter:
            ...
            try:
                send_mail(
                    subject="Password recovery in the HSE registration system",
                    message="A new password has been generated for accessing the HSE registration system. You can change it after logging in or keep using it.\n\nNew password: " + password,
                    from_email=settings.EMAIL_HOST_USER,
            ...
            messages.error(request,
                           "Account with this email not found. Maybe you mistyped it or haven't registered yet?")
    ...
    for event in events:
        ...
        for date in dates:
            ...
            for entry in entries:
                ...
                if reg.status == Registration.Status.INT and not Registration.objects.filter(guest=guest,
                                                                                             status=Registration.Status.AFF,
                                                                                             timetable__category=entry.category):
                    reg.status = Registration.Status.AFF
                    reg.save()
                    messages.success(request, "Conflicts in the category \"" + entry.category + "\" have been resolved")
...
def completed(request):
    ...
    for key, value in request.POST.dict().items():
        if "category_" in key:
            ...
            category_check = Registration.objects.filter(timetable__category=t.category, guest=guest,
                                                         status=Registration.Status.AFF)
            if unique_check:
                continue
            elif category_check:
